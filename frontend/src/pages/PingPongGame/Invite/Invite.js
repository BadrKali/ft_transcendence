...
import Waiting from '../Random/Waiting';
import PlayerInfo from '../Random/PlayerInfo';
...
const Invite = () => {
    ...
    const { data: room, isLoading: roomLoading, error: roomError } = useFetch(roomId ? `${BACKEND_URL}/api/game/invite-game-room/${roomId}` : null);
    ...
    useEffect(() => {
        ...
        socket.onopen = () => {
            socket.send(JSON.stringify({ action: 'invite' }));
        };
